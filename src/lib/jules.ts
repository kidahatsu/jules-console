...
export function getAccounts(): ProviderProfile[] {
    ...
    try {
        ...
        // Security: Validate data retrieved from localStorage using Zod to prevent insecure deserialization
        const result = z.array(ProviderProfileSchema).safeParse(parsed);
        if (!result.success) {
            console.error("Failed to validate accounts:", "Invalid format");
            return [];
        }
