...
export function getAccounts(): ProviderProfile[] {
    ...
    try {
        ...
        // Security: Validate deserialized data against schema
        const validation = z.array(ProviderProfileSchema).safeParse(migrated);
        if (!validation.success) {
            console.error("Validation failed for accounts:", validation.error.message);
            return [];
        }
        return validation.data as ProviderProfile[];
    } catch (e) {
