...
## 2025-02-18 - [MEDIUM] Fix Reverse Tabnabbing Vulnerability in Markdown Links
**Vulnerability:** The application used `react-markdown` to render external links in user-provided notes and session prompts without `target="_blank" rel="noopener noreferrer"`. This could allow linked external pages to gain access to the application's `window.opener` object, potentially leading to phishing or malicious redirections (Reverse Tabnabbing).
**Learning:** When configuring `ReactMarkdown` to add `target="_blank"` by overriding the `a` tag via the `components` prop, we must destructure the `node` prop to prevent it from being passed down to the DOM element (which causes React warnings). However, strict linting rules like `@typescript-eslint/no-unused-vars` will flag the unused `node` variable. We shouldn't use `@ts-expect-error` because it can cause 'Unused directive' build failures (TS2578) if TypeScript types align differently.
**Prevention:** Always customize the `a` tag via the `components` prop to include `target="_blank" rel="noopener noreferrer"`. Destructure the `node` prop and bypass the linter using `// eslint-disable-next-line @typescript-eslint/no-unused-vars` specifically.
