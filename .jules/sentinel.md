...
**Vulnerability:** The application was using `react-markdown` to render external links in user-provided notes and session prompts without `target="_blank" rel="noopener noreferrer"`, leaving it vulnerable to reverse tabnabbing where external sites can access `window.opener`.
**Learning:** When using `react-markdown` to render external links in an environment where we might link out to user-provided content, we must override the default `a` tag rendering to include `rel="noopener noreferrer"`.
**Prevention:** Always pass a `components` prop to `ReactMarkdown` with a custom `a` component that explicitly sets `target="_blank" rel="noopener noreferrer"`.
