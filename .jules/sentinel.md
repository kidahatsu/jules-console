...
## 2025-02-17 - [MEDIUM] Fix Reverse Tabnabbing Vulnerability in Markdown Links
**Vulnerability:** The application was rendering external links in markdown content (via `react-markdown`) without the `rel="noopener noreferrer"` attributes, exposing the application to reverse tabnabbing vulnerabilities.
**Learning:** When using `ReactMarkdown`, external links generated from markdown do not automatically include security attributes for `target="_blank"`.
**Prevention:** Always customize the `a` tag via the `components` prop in `ReactMarkdown` to explicitly include `target="_blank" rel="noopener noreferrer"`.
