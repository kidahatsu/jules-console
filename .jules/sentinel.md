...
## 2026-06-18 - [CRITICAL] Prevent Schema Validation Bypass in Profile Loads
**Vulnerability:** The application was retrieving identity profiles from `localStorage` and parsing them directly into the application state via `JSON.parse()` in `getAccounts()` without strict schema validation. An attacker with access to `localStorage` or via a compromised cross-site scripting (XSS) payload could inject arbitrary objects that do not conform to the expected `ProviderProfile` structure.
**Learning:** Data originating from `localStorage` is an untrusted boundary. Using `JSON.parse` followed by loose runtime checks or TypeScript casting does not prevent malformed objects from persisting globally in state management tools like Zustand, leading to potential application instability or downstream vulnerabilities.
**Prevention:** Always validate data retrieved from `localStorage` using strict schemas (e.g., `z.array(ProviderProfileSchema).safeParse(parsed)`) before casting or utilizing it within the application logic.
