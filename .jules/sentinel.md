...
## 2025-02-17 - [SECURITY ENHANCEMENT] Prevent Reverse Tabnabbing in Markdown Links
**Vulnerability:** Markdown links rendered by `react-markdown` were opening without `target="_blank" rel="noopener noreferrer"`. This could allow a newly opened tab to manipulate the original tab's `window.opener` object, potentially leading to phishing attacks (Reverse Tabnabbing).
**Learning:** Even sanitized markdown can be dangerous if links open in the same context or without proper protections. We must ensure external links spawned from user-generated content are isolated.
**Prevention:** Always configure `react-markdown` to override the `<a>` tag rendering via the `components` prop, forcing all links to include `target="_blank" rel="noopener noreferrer"`.
