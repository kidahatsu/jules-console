...
## 2025-02-28 - [Insecure Deserialization of Local Storage]
**Vulnerability:** Data loaded from `localStorage` (`localStorage.getItem`) was parsed using `JSON.parse` and directly cast to `ProviderProfile[]` using TypeScript `as` operator without any runtime validation. This is a form of insecure deserialization, where an attacker who can manipulate local storage (e.g. through a separate XSS vulnerability or physical access) can inject arbitrary objects, leading to potential prototype pollution or unexpected behavior if the application assumes the presence of certain properties.
**Learning:** Type casting in TypeScript only happens at compile time and offers no runtime protection. When data crosses a trust boundary (such as from `localStorage`, network requests, or user input), it must be explicitly validated at runtime.
**Prevention:** Always use a schema validation library like Zod (e.g. `z.array(ProviderProfileSchema).safeParse()`) to validate any data retrieved from external or untrusted sources before using it.
